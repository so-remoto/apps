...
export const criarPerfilComPermissoes = async (perfil: Perfil): Promise<Perfil> => {
  try {
    ...
    perfil.permissoes.forEach((permissao: Permissao) => {
...
export const atualizarPerfil = async (id: string, dadosAtualizados: Perfil): Promise<Perfil> => {
  try {
    ...
    await db.perfil.update({
      ...
      data: {
        permissoes: {
          disconnect: perfilExistente.permissoes.map((permissao: Permissao) => ({ id: permissao.id }))
        }
      }
    });
      ...
      if (!permissaoExistente) {
        const novaPermissao = await db.permissao.create({
          data: {
            ...
            dataCriacao: typeof permissao.dataCriacao === 'string'
...
export const deletarPerfil = async (id: string): Promise<void> => {
  try {
    ...
    throw new Error("Erro ao deletar perfil: " + (error instanceof Error ? error.message : 'Erro desconhecido'));
  }
};
...
export const buscarTodosPerfis = async (): Promise<Perfil[]> => {
  try {
    ...
    throw new Error("Erro ao buscar perfis: " + (error instanceof Error ? error.message : 'Erro desconhecido'));
  }
};
...
export const buscarPerfilPorId = async (id: string): Promise<Perfil | null> => {
  try {
    ...
    throw new Error("Erro ao buscar perfil por ID: " + (error instanceof Error ? error.message : 'Erro desconhecido'));
  }
};
